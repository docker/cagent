...
func NewNewCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return handleStubFallback(args, err)
			}
			...
			for event := range out {
				...
				case *runtime.ToolCallEvent:
					...
					// detect when LLM tries to write a file
					if e.ToolCall.Function.Name == "write_file" {
						fileCreated = true
					}
			...
			//NEW: fallback if no file was written
			if !fileCreated {
				return handleStubFallback(args, fmt.Errorf("no agent file was generated by the model"))
			}
			return nil
		},
	}
...
func handleStubFallback(args []string, cause error) error {
	filename := "agent.yaml"
	if len(args) > 0 {
		filename = args[0]
	}

	if stubErr := creator.WriteStubAgentFile(filename); stubErr != nil {
		return fmt.Errorf(
			"failed to generate agent: %v\n"+
				"Also failed to write stub agent file at %s: %v",
			cause, filename, stubErr,
		)
	}

	return fmt.Errorf(
		"failed to generate agent: %v\n"+
			"A stub agent file was written to %s. You can edit this file manually.",
		cause, filename,
	)
}
