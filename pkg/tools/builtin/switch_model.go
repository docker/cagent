...
var _ tools.ToolSet = (*SwitchModelToolset)(nil)
...
func (t *SwitchModelToolset) callTool(_ context.Context, params SwitchModelArgs) (*tools.ToolCallResult, error) {
	...
	// Notify the runtime about the model change
	if callback != nil {
		if err := callback(requestedModel); err != nil {
			// Rollback the internal state change since the actual switch failed
			t.mu.Lock()
			t.currentModel = previousModel
			t.mu.Unlock()
