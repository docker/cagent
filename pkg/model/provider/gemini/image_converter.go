...
var fileUploadCache = struct {
	sync.RWMutex
	cache map[string]string // localPath -> fileURI
...
func uploadOrConvertLocalFile(ctx context.Context, client *genai.Client, localPath, mimeType string) *genai.Part {
	...
	if fileURI, ok := fileUploadCache.cache[localPath]; ok {
		fileUploadCache.RUnlock()
		slog.Debug("Using cached file URI", "path", localPath, "uri", fileURI)
	...
	if client != nil {
		...
		if err == nil {
			...
			fileUploadCache.cache[localPath] = fileURI
