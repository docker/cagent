...
func TestSwitchModelTool_WithDescriptionToolSetWrapper(t *testing.T) {
	// This test verifies that the switch_model tool works correctly even when
	// wrapped in DescriptionToolSet (which happens when AddDescriptionParameter is enabled).

	// Create a switch_model toolset
	switchModelToolset, err := tools_builtin.NewSwitchModelToolset([]string{"fast", "powerful"})
	require.NoError(t, err)

	// Wrap in DescriptionToolSet (simulating what teamloader does when AddDescriptionParameter is true)
	wrappedToolset := tools.NewDescriptionToolSet(switchModelToolset)
	...
	// Manually configure the toolset handlers (simulating what RunStream does)
	// This is where the bug was: the callback wasn't being set because
	// unwrapSwitchModelToolset didn't handle the DescriptionToolSet wrapper.
	events := make(chan Event, 10)
	rt.configureToolsetHandlers(root, events)
	...
	// Before the fix, the callback wasn't set, so the override wouldn't be applied.
	require.True(t, root.HasModelOverride(), "agent should have model override after switch")
	require.Equal(t, "openai/gpt-4o", root.Model().ID(), "model should be switched to powerful")
}
...
func (w *testInstructionWrapper) Inner() tools.ToolSet { return w.ToolSet }
